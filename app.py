...
def create_app():
    ...
    app.register_blueprint(auth_bp)  # Rutas de autenticacion
    app.register_blueprint(pdf_reception_bp)
    app.register_blueprint(chat_bp)
    app.register_blueprint(oauth_bp)  # Rutas OAuth Google Drive
    app.register_blueprint(google_drive_bp)

    # Ruta principal - requiere autenticacion
    @app.route('/')
    def index():
    ...
    def file_too_large(error):
        return jsonify({
            'error': f'Archivo demasiado grande. Maximo permitido: {Config.MAX_FILE_SIZE // (1024*1024)}MB',
            'code': 'FILE_TOO_LARGE',
            'timestamp': datetime.now().isoformat()
        }), 413
    ...
    def log_request_info():
        """Log informacion de requests"""
        if not request.path.startswith('/static'):
            print(f"[{datetime.now()}] {request.method} {request.path} - {request.remote_addr}")

    # CORS basico: permitir cualquier origen para metodos comunes
    @app.after_request
